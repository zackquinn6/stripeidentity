...
describe("parseBooqableOrderWebhook", () => {
  ...
  it("parses v4 webhooks resource shape (GET /api/4/webhooks/:id delivery format)", () => {
    const body = {
      data: {
    ...
    assert.equal(p?.event, "order.updated");
    assert.equal(p?.orderId, "8b32f593-809b-4c05-8607-487da64f4265");
    assert.equal(p?.customerId, null);
    assert.equal(identityWebhookEventEligible(p, body), true);
  });

  it("parses cart.completed_checkout webhooks resource with nested order id", () => {
    const body = {
      data: {
        type: "webhooks",
        attributes: {
          event: "cart.completed_checkout",
          resource_type: "carts",
          data: {
            order: { id: "aaaaaaaa-aaaa-4aaa-8aaa-aaaaaaaaaaaa" },
          },
        },
      },
    };
    const p = parseBooqableOrderWebhook(body);
    assert.equal(p?.kind, "native");
    assert.equal(p?.event, "cart.completed_checkout");
    assert.equal(p?.orderId, "aaaaaaaa-aaaa-4aaa-8aaa-aaaaaaaaaaaa");
    assert.equal(identityWebhookEventEligible(p, body), true);
  });

  it("parses cart.completed_checkout with flat order_id on cart data", () => {
    const body = {
      data: {
        type: "webhooks",
        attributes: {
          event: "cart.completed_checkout",
          resource_type: "carts",
          data: { order_id: "bbbbbbbb-bbbb-4bbb-8bbb-bbbbbbbbbbbb" },
        },
      },
    };
    const p = parseBooqableOrderWebhook(body);
    assert.equal(p?.orderId, "bbbbbbbb-bbbb-4bbb-8bbb-bbbbbbbbbbbb");
  });
