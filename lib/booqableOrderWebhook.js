 ...
 *
 * **Admin workflow** (dashboard): New → save as Draft → Reserve. Booqable may emit
 * `order.saved_as_draft`, `order.reserved`, and/or `order.updated` depending on the
 * action. We treat `order.updated` as eligible only after confirming the order is
 * **`reserved`** via GET /api/4/orders/:id (see Booqable API v4 + order workflow docs).
 *
...
/**
 * Events that always start identity (payload is enough; no extra order GET for gating).
 * From Booqable v4 webhook resource names (orders).
 */
...
/**
 * Events to subscribe on `POST /api/4/webhook_endpoints` so admin reserve paths are
 * delivered to Vercel. Includes `order.updated` (filtered to reserved in the handler).
 */
