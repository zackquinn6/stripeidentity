 ...
 *
 * Booqable may emit many `order.*` webhook names on lifecycle changes. This app
 * accepts any of them when the payload identifies an order, then uses GET
...
export function parseBooqableOrderWebhook(body) {
  ...
  if (event && event.startsWith("order.") && Array.isArray(data)) {
    ...
    if (orderDoc) {
      return {
        ...
        customerId: customerIdFromOrderResource(orderDoc),
      };
    }
  }
  ...
  if (
    ...
    return {
      ...
      customerId: customerIdFromOrderResource(data),
    };
  }
  ...
  if (doc && typeof doc === "object" && !Array.isArray(doc) && doc.type === "orders" && doc.id) {
    return {
      ...
      customerId: customerIdFromOrderResource(doc),
    };
  }
...
export function identityWebhookEventEligible(parsed, rawBody) {
  ...
  if (parsed.kind === "native") {
    return Boolean(parsed.event && parsed.event.startsWith("order."));
  }
