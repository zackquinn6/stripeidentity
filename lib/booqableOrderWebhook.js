 ...
 *
 * Booqable exposes **predefined** integrations (e.g. Make, Zapier): you do not register
 * arbitrary URLs inside Booqable. Use a scenario whose **HTTP** step POSTs JSON here.
 * Easiest body for that step: `{ "order": { "id": "<uuid>", "customer_id": "<uuid>" } }`
 * (same as scripts/trigger-booqable-webhook.mjs). Native v4 payloads from those tools
 * are also accepted when they preserve Booqable’s `event` + `data` shape.
 *
 * Order lifecycle (admin): New → Draft (saved) → Reserved → …
 * Relevant webhook-style event names include order.saved_as_draft, order.reserved.
 *
