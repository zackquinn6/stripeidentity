 ...
 *
 * **Native push (preferred when your API token is allowed):** subscribe with Booqable’s
 * `webhook_endpoints` API so Booqable POSTs v4 JSON to your Vercel URL when subscribed
 * events fire (e.g. order reserved — “order taken” in workflow terms).
 ...
 *
 * **If that API is blocked on your plan:** use Make/Zapier HTTP to POST the wrapped body
 * below, or use GET /api/cron/poll-orders-identity (see docs/poll-identity-cron.md).
 *
 * Wrapped body (automation or tests): `{ "order": { "id": "<uuid>", "customer_id": "<uuid>" } }`
 * (same as scripts/trigger-booqable-webhook.mjs). Native v4 payloads include `event` + `data`.
 *
 * Order lifecycle (admin): New → Draft (saved) → Reserved → …
 *
