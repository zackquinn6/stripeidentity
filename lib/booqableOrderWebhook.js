 ...
 * `GET orders/:id` on the v4 base URL to run identity only when Booqable reports `status: reserved`.
 *
...
/**
 * GET orders/:id on v4 base — canonical status for gating `order.updated` webhooks.
 *
...
export async function fetchOrderStatusFromBooqable(baseUrl, apiKey, orderId) {
  ...
  const url = `${root}/orders/${encodeURIComponent(orderId)}`;
...
export async function fetchCustomerIdForOrder(baseUrl, apiKey, orderId) {
  const root = String(baseUrl).replace(/\/$/, "");
  const url = `${root}/orders/${encodeURIComponent(orderId)}?include=customer`;
  const res = await fetch(url, {
    headers: { Authorization: `Bearer ${apiKey}` },
  });
  if (!res.ok) {
