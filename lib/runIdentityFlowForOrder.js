...
import { assertToolioBooqableBaseUrl } from "./toolioBooqableOrigin.js";
...
/**
 * Avoid duplicate sessions when webhooks or automations double-fire.
...
export async function runIdentityFlowForOrder({
  ...
  if (!apiKey) {
    return {
      ...
      httpStatus: 500,
      body: {
        error: "BOOQABLE_API_KEY not configured",
      },
    };
  }

  const BOOQABLE_BASE_URL = urlCheck.normalized;

  const customerRes = await fetch(
    `${BOOQABLE_BASE_URL}/customers/${customerId}`,
    {
