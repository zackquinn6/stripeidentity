...
const durationOptions: {
  ...
  value: '1-weekend',
  label: '1 Weekend',
  description: 'Fri AM – Mon AM',
  days: 3
}, {
  value: '2-weekend',
  label: '2 Weekends',
  description: 'Fri AM – Mon AM (next week)',
  days: 10
}, {
  value: '30-day',
  label: '30 Days',
  description: 'Full month rental',
  days: 30
}];
...
const RentalDatePicker = ({
  ...
  const rentalDays = duration === 'daily' && startDate && customEndDate ? Math.ceil((customEndDate.getTime() - startDate.getTime()) / (1000 * 60 * 60 * 24)) + 1 : selectedOption?.days || 0;
  ...
  return <div className="space-y-6">
      ...
      <div>
        ...
        <div className="grid gap-3 grid-cols-2 md:grid-cols-4">
          {durationOptions.map(option => <Card key={option.value} className={cn('cursor-pointer transition-all hover:shadow-md', duration === option.value ? 'border-primary bg-primary/5 ring-1 ring-primary' : '')} onClick={() => {
          ...
          if (option.value !== 'daily') {
            onEndDateChange(undefined);
              ...
              <CardContent className="p-3 text-center">
                ...
                {option.days && <Badge variant="secondary" className="mt-2 text-xs">
                    {option.days} days
                  </Badge>}
              </CardContent>
            </Card>)}
      ...
      <div className={cn("grid gap-4", isDailyRental ? "md:grid-cols-2" : "")}>
        <div>
          ...
          <Popover open={startCalendarOpen} onOpenChange={setStartCalendarOpen}>
            ...
            }} disabled={disabledStartDays} modifiers={{
        ...
        {isDailyRental && <div>
            ...
            <Popover open={endCalendarOpen} onOpenChange={setEndCalendarOpen}>
              <PopoverTrigger asChild>
                <Button variant="outline" className={cn('w-full justify-start text-left font-normal h-12', !customEndDate && 'text-muted-foreground')}>
                  <CalendarIcon className="mr-2 h-4 w-4" />
                  {customEndDate ? format(customEndDate, 'PPP') : <span>Pick end date</span>}
                </Button>
              </PopoverTrigger>
              <PopoverContent className="w-auto p-0 bg-popover z-50" align="start">
                <Calendar mode="single" selected={customEndDate} onSelect={date => {
              onEndDateChange(date);
              setEndCalendarOpen(false);
            }} disabled={disabledEndDays} initialFocus className={cn('p-3 pointer-events-auto')} />
              </PopoverContent>
            </Popover>
          </div>}
      </div>
      ...
      {/* Summary */}
      {startDate && calculatedEndDate && rentalDays > 0 && <div className="p-4 bg-secondary/50 rounded-lg">
          ...
          {startDate && isRushDay(startDate) && <div className="mt-3 p-2 bg-destructive/10 rounded border border-destructive/20 flex items-center gap-2">
      ...
      <div className="flex items-start gap-2 text-xs text-muted-foreground">
        ...
        <p>Rush Order Processing: We’re glad to expedite your order within 48 hours for a small $50 fee.<span className="font-medium">Rush Order Processing:</span> Orders within 48 hours require a $50 rush fee.
        </p>
      </div>
    </div>;
};
