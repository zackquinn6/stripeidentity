...
### Booqable native webhooks — webshop checkout + admin reserve

Booqable documents **order** webhook names such as `order.reserved` and **`order.updated`** ([webhook endpoints](https://developers.booqable.com/#webhook-endpoints-subscribe-to-webhook-events)). A completed webshop checkout that reserves inventory typically triggers **`order.reserved`**. In the dashboard (**New → Draft → Reserve**), Booqable may emit **`order.updated`** when the order becomes **reserved**, instead of (or in addition to) **`order.reserved`**.

`scripts/register-booqable-webhook-endpoint.mjs` subscribes only to **`order.reserved`** and **`order.updated`** (`BOOQABLE_WEBHOOK_SUBSCRIBE_ORDER_EVENTS` in `lib/booqableOrderWebhook.js`). Draft/concept saves are not subscribed, so identity does not start until the order is actually reserved. For **`order.updated`**, Vercel **GETs** `/api/4/orders/:id` and runs identity **only** when Booqable reports `status: reserved`.

If you created a Booqable endpoint earlier with draft/concept events, update that endpoint’s `events` list in Booqable to match the script (or delete and re-run the register script).
