...
| Variable | Purpose |
|----------|---------|
| `BOOQABLE_BASE_URL` | **`https://toolio-inc.booqable.com/api/4`** (required; host-only legacy URL is accepted) |
...
|---------|----------------|
| **Order created in Booqable but nothing in Vercel logs** | “Create order” alone does not call your app — only a **webhook** (or Zapier/Make) POST does. Confirm a **webhook endpoint** targets your **Vercel** URL and subscribes to **order.*** (or `cart.completed_checkout`) events. If you only saved a **draft**, this app may receive a POST but respond **`skipped` / `order_not_reserved`** until the order is **reserved** (check Vercel logs for `POST ingress` and `skipped`). |
